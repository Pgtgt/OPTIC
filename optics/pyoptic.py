# -*- coding: utf-8 -*-
# 本来の色の設定を変えた．wlPenをwlcustomPenにし，全波長帯に対し，スケーリングにより色を割り当てる．それに伴いRayのPaintも変えてある．
# 可視光線ほどの光を扱うなら，wlPenをPaintに用いる，もしくはwlPenのmax,minに700,400をあたえる
# Class Grateingを追加した
#
...
class GlassDB:
    ...
    def __init__(self, fileName='schott_glasses.csv'):
        ...
        fh = gzip.open(os.path.join(
            path, 'schott_glasses.csv.gz'), 'rb')  # バイナリ読み込み
        # about GZ https://wa3.i-3-i.info/word14165.html
    ...
    def ior(self, glass, wl):
        ...
        cache = info['ior_cache']  # キャッシュ
        if wl not in cache:

            B = list(map(float, [info['B1'], info['B2'], info['B3']]))
            C = list(map(float, [info['C1'], info['C2'], info['C3']]))
            w2 = (wl/1000.)**2
            n = np.sqrt(1.0 + (B[0]*w2 / (w2-C[0])) +
                        (B[1]*w2 / (w2-C[1])) + (B[2]*w2 / (w2-C[2])))
            cache[wl] = n
        return cache[wl]

    def transmissionCurve(self, glass):  # 透過曲線
...
# どの波長をどの　色ペン　で塗るかという気持ち　波長入れると返してくれる優れもの
...
def wlcustomPen(wlcomb, wlcomb_min, wlcomb_max):
    """wlPenを改良し，下限波長　上限波長，を４００，７００としてスケーリングし，疑似的に可視光線にする．"""
    # 可視光の範囲l1^l2
    l1 = 400
    l2 = 700
    shiftlevel = (wlcomb-wlcomb_min)/(wlcomb_max-wlcomb_min)
    wl = l1+shiftlevel*(l2-l1)  # 可視光線にスケーリングした時の偽の波長
    hue = np.clip(((l2-l1) - (wl-l1)) * 0.8 / (l2-l1), 0, 0.8)
    val = 1.0
    if wl > 700:
        val = 1.0 * (((700-wl)/700.) + 1)
...
class ParamObj(object):
    ...
    def __getitem__(self, item):
        ...
        # PySide bug: https://bugreports.qt.io/browse/PYSIDE-441
...
class Lens(Optic):
    def __init__(self, **params):
        defaults = {
            'dia': 3,  # diameter of lens
            'r1': 50.,  # positive means convex(ぷっくり), use 0 for planar
        ...
        defaults.update(params)
        d = defaults.pop('d')  # popはｄを取り出しつdefoのリストから削除
        defaults['x1'] = -d/2.
        defaults['x2'] = d/2.

        gitem = CircularSolid(brush=(100, 100, 130, 100),
                              **defaults)  # brushはただの色
    ...
    def propagateRay(self, ray):
        ...
        For the incident vector I and surface normal N（法線）, and the
        ratio of indices of refraction eta(ギリシャ文字イータ), return the refraction
        ...
        for i in [0, 1]:
            ...
            # print(ior) 1.4437425200968037 1.4439256966766556 1.444107798644075
...
class Grating(Optic):
    ...
    def __init__(self, **params):
        defaults = {
            ...
            'r2': 0,
            'd': 0.01,
            'd_Grate': 1.6e3,  # nm/line
            'm': +1,  # -1なら，ぶつかって右へそれる．+1ならひだり
        }
        defaults.update(params)
        d = defaults.pop('d')
        defaults['x1'] = -d/2.
        defaults['x2'] = d/2.
        gitem = CircularSolid(brush=(100, 200, 100, 255), **defaults)
        Optic.__init__(self, gitem, **defaults)
    ...
    def propagateRay(self, ray):
        ...
        if p1 is not None:
            p1 = surface.mapToItem(ray, p1)
            rd = ray['dir']  # 絶対入社ベクトル
            a1 = np.arctan2(rd[1], rd[0])  # a1は絶対入射角rad
            a_incre_grate = np.arcsin(
                (self['m']*(ray['wl'])/self['d_Grate'])-np.sin(ai))
            a_abs_grate = a1-ai+a_incre_grate-np.pi  # ar絶対反射角ai相対入射角
            ray.setEnd(p1)
            dp = Point(np.cos(a_abs_grate), np.sin(a_abs_grate))
            ray = Ray(parent=ray, dir=dp)
        else:
            ray.setEnd(None)
        return [ray]
...
class Block(Optic):  # d_Grate [nm] #Opticを継承している
    def __init__(self, **params):
        defaults = {
            'r1': 0,
            'r2': 0,
        ...
        defaults['x1'] = -d/2.
        defaults['x2'] = d/2.
        gitem = CircularSolid(brush=(200, 200, 200, 255), **defaults)
...
class CircularSolid(pg.GraphicsObject, ParamObj):
    ...
    def __init__(self, pen=None, brush=None, **opts):
        ...
        """
        defaults = dict(x1=-2, r1=100, d1=20, x2=2, r2=100, d2=20)
        defaults.update(opts)
        ParamObj.__init__(self)
        self.surfaces = [CircleSurface(
            defaults['r1'], defaults['d1']), CircleSurface(-defaults['r2'], defaults['d2'])]
...
class CircleSurface(pg.GraphicsObject):
    ...
    def intersectRay(self, ray):
        ...
        h = self.h2
        r = self.r
        # position and angle of ray in local coords.
        ...
        if r == 0:
            ...
            # ic(pt + Point(r, 0), ang-norm)
            # pt + Point(r, 0), ang-norm 相対位置か
            # ic(pt, ang-norm)
            # ic(pt + Point(r, 0), ang-norm, ang, norm, Point(r,0))
            return pt + Point(r, 0), ang-norm


class Ray(pg.GraphicsObject, ParamObj):
    """Represents a single straight segment of a ray
    How to use (ex)   Ray(start=Point(-50, dy+2*scany), dir=(1, 0), wl=780))"""
    sigStateChanged = QtCore.Signal()

    def __init__(self, **params):
        ParamObj.__init__(self)
        defaults = {
            'ior': 1.0,  # 屈折率（Index of Refraction）
            'wl': 1550,
            'wl_max': 1600,
            'wl_min': 1500,
            'end': None,
    ...
    def paint(self, p, *args):
        ...
        # p.setPen(wlcombPen(self['wl'], 1400, 1700))
        p.setPen(wlcustomPen(self['wl'], wlPenStart, wlPenStop))
...
def trace(rays, optics):
    ...
    if len(optics) < 1 or len(rays) < 1:  # op rayのどっちかが０だったら虚無
        return
    for r in rays:
        # print(r["start"])
        r.clearChildren()
...
class Tracer(QtCore.QObject):
    ...
    def trace(self):
        ...
        for rr in self.rays:
            pass
            # print(rr["start"],"tracer")
        # for rr in self.rays:
        #     ic(rr["start"],"tracer")
        # raynum = len(self.rays)
        # ic(raynum)
        # for raynum in range(len(self.rays)):